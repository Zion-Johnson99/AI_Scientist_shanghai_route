...
def audit_route(route: dict[str, Any], index: int) -> dict[str, Any]:
    ...
    segment_lengths = [distance_m(first, second) for first, second in itertools.pairwise(points)]
...
def retrace_metrics(points: list[Point], lengths: list[float]) -> tuple[float, float]:
    keys = [_edge_key(first, second) for first, second in itertools.pairwise(points)]
    counts = Counter(keys)
...
def point_polyline_distance_m(point: Point, points: list[Point]) -> float:
    return min(point_segment_distance_m(point, first, second) for first, second in itertools.pairwise(points))
