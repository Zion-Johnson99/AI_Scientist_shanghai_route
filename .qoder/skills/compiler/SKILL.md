...
`references/figure-extraction-guide.md`（相对于本 Skill 目录）。
...
`references/ara-schema.md`（相对于本 Skill 目录）for field-level format.
...
- `references/exploration-tree-spec.md` — exploration tree YAML spec
- `references/validation-checklist.md` — all Seal Level 1 checks
- `references/figure-extraction-guide.md` — reading plots/diagrams/samples + PyMuPDF render/crop recipes; load when an input has figures whose information is only visual
