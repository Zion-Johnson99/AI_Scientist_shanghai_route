...
description: Render an Agent-Native Research Artifact (ARA) into one self-contained interactive HTML file showing the research process, exploration branches and dead ends, claims, evidence, grounded results, figures, tables, and artifact pointers. Use for requests to visualize an ARA, paper, repository, experiment run, research trajectory, process map, or step-by-step workflow. Existing ARAs are read-only inputs; raw papers, repositories, logs, or notes are compiled before visualization.
