...
```bash
node ".qoder/skills/web-access/scripts/check-deps.mjs"
```
...
按脚本输出处理：
- `exit 0` → 继续
- `exit 2` → 需询问用户偏好，写入 `.qoder/skills/web-access/config.env` 的 `WEB_ACCESS_BROWSER`
...
```bash
node ".qoder/skills/web-access/scripts/find-url.mjs" [关键词...] [--only bookmarks|history] [--browser chrome|edge] [--limit N] [--since 1d|7h|YYYY-MM-DD] [--sort recent|visits]
```
