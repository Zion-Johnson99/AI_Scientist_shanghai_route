async (page) => {
  ...
  await page.evaluate(async () => {
    ...
    const routes = (await response.json()).filter((route) => route.route_mode === "run");
    ...
    window.__runAudit = { routes, auditMap, AMapApi };
  });
  const colors = ["#e53935", "#00a884", "#7c3aed", "#f59e0b", "#1677ff"];
  await page.evaluate((palette) => {
    window.__showRunAuditBatch = (batchIndex) => {
      const { routes, auditMap, AMapApi } = window.__runAudit;
      auditMap.clearMap();
      ...
      document.getElementById("auditTitle").textContent =
        `跑步路线视觉复核 · 第 ${batchIndex + 1}/6 批 · ${selected.map((route) => route.route_id.slice(-4)).join(" / ")}`;
    };
  }, colors);
  for (let batch = 0; batch < 6; batch += 1) {
    await page.evaluate((index) => window.__showRunAuditBatch(index), batch);
    await page.waitForTimeout(1800);
    await page.screenshot({
      path: `output/playwright/run-audit-batch-${batch + 1}.png`,
      fullPage: true,
    });
  }
  await page.evaluate((palette) => {
    window.__showRunAuditRoute = (routeIndex) => {
      const { routes, auditMap, AMapApi } = window.__runAudit;
      auditMap.clearMap();
      ...
      document.getElementById("auditTitle").textContent =
        `跑步路线视觉复核 · ${route.route_id} · ${(route.actual_distance_m / 1000).toFixed(2)} km · ${route.route_shape}`;
    };
  }, colors);
  const routeImages = [];
  for (let routeIndex = 0; routeIndex < 30; routeIndex += 1) {
    await page.evaluate((index) => window.__showRunAuditRoute(index), routeIndex);
    await page.waitForTimeout(650);
    const imagePath = `output/playwright/run-audit-${String(routeIndex + 31).padStart(4, "0")}.png`;
