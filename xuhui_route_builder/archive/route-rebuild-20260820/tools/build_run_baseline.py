...
OUTPUT_PATH = PROJECT_ROOT / "data/processed/run_rebuild_baseline_20260820.json"
QUALITY_GATE_PATH = (
    REPO_ROOT / ".agents/skills/optimize-xuhui-routes/scripts/route_quality_gate.py"
)

PLANNED_GEOMETRY_CHANGED = {
    "XH_RUN_0033",
    "XH_RUN_0034",
    "XH_RUN_0036",
    "XH_RUN_0041",
    ...
    preference_hits = sorted(
        {
            poi["poi_type"]
            for poi in verified
            if poi.get("poi_type") in {"coffee", "park_gate", "toilet", "convenience"}
        }
    )
    supply_warning = float(route.get("actual_distance_m", 0)) > 5_000 and not (
...
def _classification(route_id: str) -> str:
    if route_id in PLANNED_GEOMETRY_CHANGED:
        return "geometry_changed"
    if route_id in METADATA_ONLY:
...
def _load_quality_gate():
    spec = importlib.util.spec_from_file_location(
        "run_baseline_quality_gate", QUALITY_GATE_PATH
    )
...
def build_baseline() -> dict[str, Any]:
    ...
    runs = [route for route in candidates if route.get("route_mode") == "run"]
    if len(runs) != 30:
        raise RuntimeError(f"expected 30 run routes, got {len(runs)}")
    ...
    records = [
        ...
        for index, route in enumerate(runs)
    ]
    return {
        "metadata": {
            "route_mode": "run",
