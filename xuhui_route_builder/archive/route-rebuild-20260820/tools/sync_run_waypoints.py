...
RESEARCH_PATH = PROJECT_ROOT / "data/seeds/research/run_route_optimization_0815.json"
...
def _run_routes(routes: list[dict[str, Any]]) -> list[dict[str, Any]]:
    selected = [route for route in routes if route.get("route_mode") == "run"]
    if len(selected) != 30:
        raise ValueError(f"expected 30 run routes, got {len(selected)}")
    return selected
...
def _assert_real_node_names(routes: list[dict[str, Any]]) -> None:
    ...
    if invalid:
        raise ValueError(f"run routes contain placeholder nodes: {', '.join(invalid)}")
...
def _sync_metadata_only_targets(candidates: list[dict[str, Any]]) -> None:
    route = next(
        (
            ...
            if item.get("route_mode") == "run" and item.get("route_id") == "XH_RUN_0046"
        ),
        None,
    )
    if route is None:
        raise ValueError("missing metadata-only route XH_RUN_0046")
...
def _sync_seed_nodes(
    ...
    run_routes = _run_routes(candidates)
    run_seeds = _run_routes(seeds)
    _assert_real_node_names(run_routes)
    route_id_by_seed_id = {
        record["seed_id"]: record["route_id"]
        for record in research_routes or []
        if record.get("seed_id") and record.get("route_id")
    }
    for route, seed in zip(run_routes, run_seeds, strict=True):
        seed_route_id = seed.get("route_id") or route_id_by_seed_id.get(
            seed.get("seed_id")
        )
        if route["route_id"] != seed_route_id:
            raise ValueError(
                f"run route order mismatch: {route['route_id']} != {seed_route_id}"
            )
...
def _sync_research_nodes(
    ...
    if len(research_routes) != 30:
        raise ValueError(f"expected 30 run research routes, got {len(research_routes)}")
    by_id = {route["route_id"]: route for route in run_routes}
...
def _sync_candidate_pois(
    ...
    published_by_id = {
        ...
        if route.get("route_mode") == "run"
    }
    for candidate in candidates:
        if candidate.get("route_mode") != "run":
            continue
        route_id = candidate["route_id"]
        source = published_by_id.get(route_id)
        if source is None:
            raise ValueError(f"missing published run route: {route_id}")
        if candidate.get("actual_distance_m") != source.get(
            "actual_distance_m"
        ) or candidate.get("polyline_gcj02") != source.get("polyline_gcj02"):
            raise ValueError(f"run route geometry mismatch: {route_id}")
        for field in (
            "nearby_pois",
            "amenity_ids",
            "preference_hits",
...
def main() -> None:
    ...
    print("run waypoints synchronized: 30 routes")
