...
def _point(lng: float, lat: float) -> CoordinatePair:
    return CoordinatePair(
        lng_gcj02=lng + 0.005, lat_gcj02=lat - 0.002, lng_wgs84=lng, lat_wgs84=lat
    )
...
def _route(
    ...
    *,
    ...
    start = RouteLocation(
        ...
        name="测试路线入口" if is_loop else "测试路线终点",
        location_type="public_space",
...
def test_validate_candidate_accepts_only_complete_supported_geometry() -> None:
    ...
    checked = validate_candidate(
        route, _overpass(), datetime(2026, 7, 11, tzinfo=timezone.utc), "osm-test"
    )
...
def test_validate_candidate_accepts_amap_local_visual_evidence_without_osm() -> None:
    points = [_point(121.44, 31.18), _point(121.441, 31.18)]
    route = _route(points)
    ...
    checked = validate_candidate(
        ...
        "amap_walking_v2+local_topology+visual_audit_2026-08-19",
    )

    assert checked.validation_status == "accepted"
    assert checked.snap_ratio is None
    assert checked.is_publishable()
    assert "高德原始路径" in checked.review_note
    assert "本地几何" in checked.review_note
    assert "全景目视" in checked.review_note
...
def test_validate_candidate_requires_endpoints_and_ninety_percent_inside_xuhui() -> (
    None
):
    boundary = [
        [121.439, 31.179],
...
def test_validate_amap_raw_evidence_requires_real_successful_direction_response(
    tmp_path: Path,
) -> None:
    points = [_point(121.44, 31.18), _point(121.441, 31.18)]
    route = _route(points)
    ...
    raw_path.write_text(
        json.dumps({"status": "0", "route": {"paths": []}}), encoding="utf-8"
    )
    failures = validate_amap_raw_evidence(route, tmp_path)
    assert any("status" in failure for failure in failures)


def test_raw_evidence_segments_are_independent_from_display_waypoints(
    tmp_path: Path,
) -> None:
    points = [_point(121.44, 31.18), _point(121.441, 31.18)]
    route = _route(points)
    ...
    raw_dir.mkdir(parents=True)
    paths = []
    half_distance = route.actual_distance_m // 2
    for index, (start, end, distance) in enumerate(
        [
            ("121.445,31.178", "121.4455,31.178", half_distance),
            (
                "121.4455,31.178",
                "121.446,31.178",
                route.actual_distance_m - half_distance,
            ),
        ]
    ):
        path = raw_dir / f"bicycling_v2_segment_{index}.json"
        path.write_text(
            json.dumps(
                {
                    "status": "1",
    ...
    route = route.model_copy(
        update={"raw_response_paths": paths, "waypoint_names": ["真实起点", "真实终点"]}
    )

    assert validate_amap_raw_evidence(route, tmp_path) == []


def test_validate_amap_raw_evidence_rejects_missing_or_outside_paths(
    tmp_path: Path,
) -> None:
...
@pytest.mark.parametrize(
    "failure", ["missing_osm", "low_snap", "distance_error", "zero_duration"]
)
def test_validate_candidate_sends_unverified_routes_to_review(failure: str) -> None:
    ...
    checked = validate_candidate(
        route, payload, datetime(2026, 7, 11, tzinfo=timezone.utc), "osm-test"
    )
    assert checked.validation_status == "needs_review"
...
def test_validate_candidate_stably_reviews_invalid_validation_inputs(
    payload, verified_at, network_version
) -> None:
...
def test_find_duplicate_routes_treats_reverse_and_densified_backbone_as_same() -> None:
    ...
    dense = [
        _point(121.44, 31.18),
        _point(121.4405, 31.18),
        _point(121.441, 31.18),
        _point(121.442, 31.18),
    ]
    unique = [_point(121.44, 31.18), _point(121.44, 31.181)]
    duplicates = find_duplicate_routes(
        [
            _route(base, route_id="base"),
            _route(reverse, route_id="reverse"),
            _route(dense, route_id="dense"),
            _route(unique, route_id="unique"),
        ]
    )
...
def test_duplicate_routes_support_closed_loop_rotation_and_high_overlap() -> None:
    ...
    mostly_same = [
        _point(121.44, 31.18),
        _point(121.441, 31.18),
        _point(121.441, 31.1811),
        _point(121.44, 31.18),
    ]
    duplicates = find_duplicate_routes(
        [
            _route(loop, route_id="loop"),
            _route(rotated, route_id="rotated"),
            _route(mostly_same, route_id="overlap"),
        ]
    )
...
def test_duplicate_routes_groups_transitive_similarity_chain(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda first, second, tolerance: (
            1.0 if frozenset({first.route_id, second.route_id}) in overlaps else 0.0
        ),
    )
    duplicates = find_duplicate_routes(
        [
...
def test_overpass_client_caches_and_wraps_request_errors(tmp_path) -> None:
    ...
    with pytest.raises(RuntimeError, match="Overpass request failed"):
        OverpassClient(cache_dir=tmp_path / "broken", session=BrokenSession()).query(
            "new-query"
        )
    ...
    assert OverpassClient(
        cache_dir=tmp_path / "get", session=GetSession(), method="get"
...
def test_overpass_client_sends_meaningful_user_agent(tmp_path) -> None:
    ...
    class Session:
        def post(self, *args, **kwargs):
            ...
            assert (
                "github.com/Zion-Johnson99/AI_Scientist_shanghai_route"
                in kwargs["headers"]["User-Agent"]
            )
            return Response()
