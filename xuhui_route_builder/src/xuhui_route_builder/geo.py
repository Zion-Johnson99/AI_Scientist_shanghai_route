...
def parse_lng_lat(value: str) -> CoordinatePair:
    ...
    return CoordinatePair(lng_gcj02=lng, lat_gcj02=lat, lng_wgs84=wgs_lng, lat_wgs84=wgs_lat)
...
def gcj02_to_wgs84(lng: float, lat: float) -> tuple[float, float]:
    ...
    mg_lat = lat + dlat
    mg_lng = lng + dlng
    return round(lng * 2 - mg_lng, 6), round(lat * 2 - mg_lat, 6)
...
def parse_amap_boundary(response: dict[str, Any]) -> dict[str, Any]:
    ...
    ring = [[float(lng), float(lat)] for lng, lat in (point.split(",", 1) for point in polygon_text.split(";"))]
    if ring[0] != ring[-1]:
        ring.append(ring[0])
