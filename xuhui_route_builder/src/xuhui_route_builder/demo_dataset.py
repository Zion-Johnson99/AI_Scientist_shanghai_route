...
def _build_routes() -> list[CandidateRoute]:
    routes: list[CandidateRoute] = []
    route_index = 1
    for zone_index, zone in enumerate(ZONE_SPECS):
        for mode, target_m, repeat in ROUTE_PLAN:
            for variant in range(repeat):
                routes.append(_make_route(route_index, zone_index, zone, mode, target_m, variant))
                route_index += 1
        if zone_index < 3:
            routes.append(_make_route(route_index, zone_index, zone, "bike", 10000, 8))
            route_index += 1
    if len(routes) != 150:
...
def _make_route(index: int, zone_index: int, zone: ZoneSpec, mode: str, target_m: int, variant: int) -> CandidateRoute:
    prefix = "RUN" if mode == "run" else "WALK" if mode == "walk" else "BIKE"
    distance_error = ((variant % 5) - 2) * max(30, target_m // 80)
    actual_m = target_m + distance_error
    duration_s = int(actual_m / (3.1 if mode == "bike" else 2.0 if mode == "run" else 1.25))
    loop_flag = mode != "bike" and target_m <= 5000
    polyline = _route_polyline(zone.anchor, zone_index, target_m, variant, loop_flag)
    start_entry_id, end_entry_id = ZONE_ENTRY_IDS[zone.name]
    tags = list(zone.tags[:2]) + [_distance_label(target_m), PREFERENCE_TAGS[(index - 1) % len(PREFERENCE_TAGS)]]
    route = CandidateRoute(
        route_id=f"XH_{prefix}_{index:04d}",
        route_name=_route_name(zone.name, mode, target_m, variant),
        route_mode=mode,
        ...
        tags=tags,
        source_method="curated_amap_ready",
        road_names=_road_names(zone.name),
        turn_count=4 + (variant % 5),
        route_inside_ratio=round(0.88 + (variant % 8) * 0.01, 2),
        source_name=zone.source_name,
        source_url=zone.source_url,
        confidence=zone.confidence,
        distance_error_m=distance_error,
        loop_flag=loop_flag,
        feature_tags=list(zone.tags),
        candidate_rank="recommended" if variant == 0 else "convenient" if variant == 1 else "candidate",
    )
    return attach_score_placeholder(route)


def _route_polyline(anchor: tuple[float, float], zone_index: int, target_m: int, variant: int, loop_flag: bool) -> list[tuple[float, float]]:
    lng, lat = anchor
    scale = target_m / 1000
    dx = 0.0018 + scale * 0.00022
    dy = 0.0012 + scale * 0.00016
    bias = (variant - 2) * 0.00055
    slant = (zone_index % 3 - 1) * 0.00045
    points = [
        (lng + bias, lat - dy * 0.30),
        (lng + dx * 0.55 + bias, lat + dy * 0.20 + slant),
        (lng + dx * 0.95 + bias * 0.5, lat + dy * 0.82),
        (lng + dx * 0.20 - bias, lat + dy * 1.10 - slant),
        (lng - dx * 0.65 - bias * 0.5, lat + dy * 0.55),
        (lng - dx * 0.45 + bias * 0.3, lat - dy * 0.15),
    ]
    if loop_flag:
