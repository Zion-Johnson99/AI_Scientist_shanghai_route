...
以下内容可直接复制到 Qoder 编程模式的 Goal 任务输入框。开始前确认工作区为 `AI_Scientist_develop`、运行方式为本地模式、分支为 `Qwen_Harness_Build`、权限为完全访问、模型为 `Qwen3.8-Max`，并启用 Goal。施工会话沿用界面选择的模型；生成后的 Harness 调用 Qwen API 时默认使用 `medium`。本版本采用快速一次性施工模式，目标是在一个连续 Goal 任务中完成全部生产代码，测试与正式验收转入后续 QA 阶段。
...
1. 使用一个主 Goal 任务连续完成仓库盘点、代码施工、集成和交接，中途不等待阶段确认。
...
5. 当前客户端若未提供同一任务内的六子代理编排能力，主代理按上述六个角色和两轮文件所有权连续完成，使用当前可用并发且仍不超过 3 个；代理能力差异不得成为提前结束理由。
6. 主代理在每轮开始前划定互斥文件范围。子代理只编辑分配文件，公共模型和共享入口由主代理最终合并。
7. 本轮只完成生产代码、配置、Schema、Prompt、Skills、示例 fixture 和必要文档。新测试、CI workflow、浏览器验收、真实 API smoke、Ruff、Pyright 与完整项目测试统一留给后续 QA 阶段。
8. 最终只运行低成本生成期检查：必需文件完整性、JSON 可解析性、Python 语法编译、新增 JavaScript 语法和 git diff --check。该检查不等同于正式测试。
9. 不创建阶段提交，不推送远程。全部代码完成后保留工作树改动，交给后续 Codex QA、补测、修复和提交。
10. 最终状态统一标记为 implementation_complete_unverified，避免把未测试代码描述为已经验收。
11. 只有真实 Key、外部付费数据、生产部署授权、路线批量重建或未知工作树冲突会阻塞施工；单个子任务受阻时，主代理继续推进其他互不依赖任务并记录阻塞项。
...
十、完成定义：
1. `Qwen-Harness/` 已包含五份权威文档定义的生产源码、配置、Schema、Prompt、示例 fixture、CLI 入口和必要说明。
2. `.qoder/skills/` 已包含六个项目专属 Skill；每个 Skill 都有完整 `SKILL.md`，并按 02 文档补齐需要的 references 与确定性辅助脚本。
3. `evaluation_model_qwen` 的 `score-candidates` 窄接口和 `xuhui_route_builder` 的科研结果面板接线已经实现，原有公开行为保持稳定。
4. 生产实现中没有用空函数、`pass`、伪代码或待办标记代替文档已经定义的功能。
5. 必需文件完整性、JSON 解析、Python 语法编译、新增 JavaScript 语法和 `git diff --check` 均完成且通过。
6. 仅测试、真实 API、部署、提交和推送留给后续 QA；任何生产实现缺口都列为未完成并继续施工。
...
现在从阶段 0 开始；阶段 0 仅完成真实仓库盘点与基线结构验证，跳过基线测试。随后无需等待用户确认，按阶段 1 至阶段 8 的依赖顺序和上述两轮调度连续执行，直至“十、完成定义”全部满足并输出 implementation_complete_unverified 报告。
```
...
把上面的整段提示词发送给 Qoder 编程模式的 Goal 任务。Goal 会围绕完成定义持续推进仓库盘点、两轮子代理施工、主代理集成和最终交接，无需在阶段之间手工发送“继续”。
